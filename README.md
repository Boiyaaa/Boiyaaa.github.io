...
纯静态个人学术主页。无需安装依赖，无需构建。双击 index.html 即可查看。支持手机、中英文切换和深浅色切换。

## 发布到 GitHub Pages

1. 在 GitHub 创建名为 `Boiyaaa.github.io` 的公开仓库（已有则使用原仓库，先备份旧内容）。
2. 将本文件夹内的 `index.html`、`styles.css`、`script.js`、`favicon.svg`、`.nojekyll` 上传到仓库根目录，不要把外层文件夹整体上传。
...
- `styles.css`：页面样式、手机适配和减少动画偏好，顶部变量控制颜色。
- `script.js`：语言切换、主题切换、年份和滚动导航高亮。语言和主题选择保存在浏览器中；存储不可用时仍可正常切换。

现在使用 E 字母标记作为头像。替换 `.portrait` 中的 span 为自己的 img，并设置 width:100%;height:100%;object-fit:cover 即可。
